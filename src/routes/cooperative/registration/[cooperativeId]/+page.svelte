<script lang="ts">
  ...
  import type { MemberAccount } from "$lib/definitions/types.js";
    ...
    export let data;
    let message = ""
    const {form, data: body, errors, isSubmitting} = createForm<MemberAccount>({
...
<div class="bg-gray-50 flex justify-center items-center" style="min-height:100vh;">
    ...
    <div class="bg-base-100 p-3 pb-6 shadow rounded ">
        ...
        <form use:form>
            ...
            <TextField name="member.givenName" label="Given name" labelFor="givenName"  error={$errors?.member?.givenName?.[0]}/>
            <TextField name="member.middleName" label="Middle name" labelFor="middleName" error={$errors?.member?.middleName?.[0]}/>
            <TextField name="member.surname" label="Surname" labelFor="surname"  error={$errors?.member?.surname?.[0]}/>
            <TextField name="member.birthday" label="Date of birth" labelFor="birthday" type="date"  error={$errors?.member?.birthday?.[0]} />
              </div>
            <div class="mt-5"></div>
            ...
            <button class="btn btn-primary mt-3 text-white w-full" type="submit" disabled={$isSubmitting}> Sign Up</button>
        </form>
    </div>
    </div>
