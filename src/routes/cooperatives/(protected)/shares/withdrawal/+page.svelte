<script lang="ts">
  ...
  import { StatusCodes } from "http-status-codes";
    ...
    const edit = async(log: Share)=>{
      ...
      editFormBody.update((prev)=>({...prev, memberId: log.member?.id ?? 0, amount: log.amount, remarks: log.remarks, id: log.id ?? 0, share: data?.member.share,}))
    
      selectedMember = log.member
      if(selectedMember){
        selectedMember.share = data?.member?.share ?? 0
      }
