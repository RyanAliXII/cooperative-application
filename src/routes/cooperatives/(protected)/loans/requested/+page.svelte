<script lang="ts">
  ...
  import {
    AddLoanSchemaValidation,
    EditLoanSchemaValidation,
  ...
  const {
    form: editLoanForm,
    data: editLoanData,
    errors: editLoanFormErrors,
    reset: resetEditLoanForm,
  } = createForm({
    initialValues: {
      id: "",
      memberId: 0,
    ...
    extend: [
      validator({
        schema: EditLoanSchemaValidation,
        level: "error",
        castValues: true,
      }),
    ],
...
<div>
  ...
  <div class="container bg-base-100 w-full p-3 rounded mb-8 h-56 flex">
    ...
    >
      ...
      <h2 class="text-3xl font-bold">
        PHP {data.total.principal.toLocaleString(undefined, MONETARY)}
      </h2>
    ...
    >
      ...
      <h2 class="text-3xl font-bold">
        PHP {data.total.interest.toLocaleString(undefined, MONETARY)}
      </h2>
      <p>Total Interest</p>
    </div>
  </div>
  <div class="container bg-base-100 w-full p-3 rounded">
    ...
    >
      <i class="fa-solid fa-plus mr-1" />Add Loan</button
    >
      ...
      <table class="table w-full">
        ...
        <tbody>
          {#each data.loans as loan}
            <tr>
              <td
                >{loan.member?.givenName}
                {loan.member?.middleName}
                {loan.member?.surname}</td
              >
              <td
              ...
              >
              <td
                >{loan.interest.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}</td
              >
              <td
                >{(loan.principal + loan.interest).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}</td
              >
              <td
  ...
  <form use:addLoanForm>
    {#if selectedMember}
      <div class="flex mt-3 gap-2 w-full bg-white shadow-sm p-5 border">
        <div>
          <img
            src="https://api.dicebear.com/6.x/initials/svg?seed={selectedMember?.givenName} {selectedMember?.surname}&backgroundColor=EB7C2A"
            alt="avatar"
        ...
        <div class="flex flex-col justify-center">
          <span class="font-bold"
            >{selectedMember?.givenName} {selectedMember?.surname}</span
          >
          <small class="text-gray-400">{selectedMember?.id}</small>
        </div>
        <div
          class="text-error cursor-pointer flex items-center flex-1 justify-end gap-0.5"
          on:click={removeSelectedMember}
          role={"button"}
        >
          <i class="fa-solid fa-xmark text-lg" /> Remove
        </div>
      </div>
    {:else}
      <AccountSelector
        handleSelect={selectMemberForAddLoan}
        error={$addLoanFormErrors?.memberId?.[0]}
      />
    {/if}
    <TextField
      label="Amount"
...
>
  ...
  <form use:editLoanForm>
    ...
    {:else}
      <AccountSelector
        handleSelect={selectMemberForAddLoan}
        error={$editLoanFormErrors?.memberId?.[0]}
      />
    {/if}
    <TextField
      ...
      type="number"
      step={0.01}
      error={$editLoanFormErrors?.amount?.[0]}
    />
    <TextField
      value={$editLoanData.interest}
      label="Interest Rate(%)"
      name="interest"
      type="number"
      min="1"
      error={$editLoanFormErrors?.interest?.[0]}
    />
    <TextField
      value={$editLoanData.tenure}
      label="Tenure(In Months)"
      name="tenure"
      type="number"
      min="1"
      error={$editLoanFormErrors?.tenure?.[0]}
    />
    <div
      class="bg-base-200 w-full h-10 rounded flex items-center px-2 text-gray-600 font-semibold gap-2 mt-5"
    >
    ...
    <div class=" w-full h-30 rounded p-5">
      <div class="grid grid-cols-2 border-b py-2">
        <span>Principal</span>
        <span class="font-bold"
          >{$editLoanData?.amount?.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }) ?? 0}</span
        >
      ...
      <div class="grid grid-cols-2 border-b py-2">
        <span>Interest</span>
        <span class="font-bold"
          >{editLoanInterest?.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}</span
        >
      ...
      <div class="grid grid-cols-2 border-b py-2">
        <span>Total Due</span>
        <span class="font-bold"
          >{editLoanTotalDue?.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}</span
        >
      ...
      <div class="px-2">
        <div class="grid grid-cols-2 py-2 border-b">
          ...
          <span class="font-bold"
            ...
            !isFinite(editLoanRepaymentPrincipal)
              ? (0).toLocaleString(undefined, { minimumFractionDigits: 2 })
              : editLoanRepaymentPrincipal?.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}</span
          >
        ...
        <div class="grid grid-cols-2 py-2 border-b">
          ...
          <span class="font-bold"
            ...
            !isFinite(editLoanRepaymentPrincipal)
              ? (0).toLocaleString(undefined, { minimumFractionDigits: 2 })
              : editLoanRepaymentInterest?.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}</span
          >
        ...
        <div class="grid grid-cols-2 py-2 border-b">
          ...
          <span class="font-bold"
            ...
            !isFinite(editLoanRepaymentPrincipal)
              ? (0).toLocaleString(undefined, { minimumFractionDigits: 2 })
              : editLoanRepaymentTotalDue.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}</span
          >
...
>
  <div class="flex mt-3 gap-2 w-full bg-white shadow-sm p-5 border">
    <div>
      <img
        src="https://api.dicebear.com/6.x/initials/svg?seed={selectedLoan
          ?.member?.givenName} {selectedLoan?.member
          ?.surname}&backgroundColor=EB7C2A"
        alt="avatar"
        class="w-10 rounded-full"
      />
    </div>
    <div class="flex flex-col justify-center">
      <span class="font-bold"
        >{selectedLoan?.member?.givenName}
        {selectedLoan?.member?.middleName}
        {selectedLoan?.member?.surname}</span
      >
  ...
  <div class=" w-full h-30 rounded p-5 flex flex-col gap-1">
    ...
    <div class="w-full flex gap-3 mt-10">
      <button
        ...
        type="button"
        on:click={approveLoan}>Approve</button
      >
      <button
        class="btn btn-error basis-1/2"
        type="button"
        on:click={declineLoan}>Decline</button
      >
    </div>
  </div></Modal
>
...
<ConfirmDialog
  ...
  text={"Are you sure you want to delete is this loan request?"}
