<script lang="ts">
  ...
  import { SavingsTransactionTypes, SharesTransactionTypes } from "$lib/internal/transaction";
  import { MONETARY } from "$lib/internal/config";
    ...
    const fallbackStat:CooperativeStats = {assets:0, cooperativeId:"", liquidity:0, loan:0, 
          cooperativeName:"", loanInterest: 0, members:0, registrationFees:0, savings:0, savingsPrincipal: 0, shares: 0,  sharesPrincipal: 0,withdrawnSavings:0, withdrawnShares:0 }
    </script>
    ...
    <div>
        ...
        <div class="tabs w-full">
          ...
          <button  class="tab tab-lifted" class:tab-active={activeTab === "overview"}  on:click={()=>{activeTab = "overview"}}>Overview</button> 
        ...
        <div class="container bg-base-100 w-full  p-3 rounded">
          <div class="mb-10 mt-10 flex items-center gap-5 ml-3">
            <img src="https://api.dicebear.com/6.x/initials/svg?seed={data?.cooperative.name}&backgroundColor=EB7C2A" alt="avatar" class="w-12 rounded-full">
            <div>
              <h1 class="text-lg font-bold">{data?.cooperative?.name}</h1>
              <small class="text-gray-500">Cooperative ID: {data?.cooperative.id}</small>
            </div>
          </div>
          {#if isViewMode }
          ...
          {/if}
            ...
            <div class="mt-5 w-full flex justify-end">
                 <button class=" btn btn-primary px-8 py-2 mr-2 mb-2 text-base-100" disabled={ isViewMode ? true : false} >
        ...
        <div class="container bg-base-100 w-full  p-3 rounded">
          <div class="bg-base-200 w-full h-10 rounded flex items-center px-2 text-gray-600 font-semibold gap-2 mt-5">
           ...
           <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
            ...
            <span class="font-bold">PHP {data?.stat?.savingsPrincipal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
           </div>
           <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
            <span>Savings Withdrawal</span>
            <span class="font-bold">PHP {data?.stat?.withdrawnSavings.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
           </div>
           <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
        ...
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          ...
          <span class="font-bold">PHP {data?.stat?.sharesPrincipal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
        
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          <span>Shares Withdrawal</span>
          <span class="font-bold">PHP {data?.stat?.withdrawnShares.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
        ...
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          <span>Loan</span>
          <span class="font-bold">PHP {data?.stat?.loan.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          <span>Loan Interest</span>
          <span class="font-bold">PHP {data?.stat?.loanInterest.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
        <div class="bg-base-200 w-full h-10 rounded flex items-center px-2 text-gray-600 font-semibold gap-2 mt-5">
        ...
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          <span>Assets</span>
          <span class="font-bold">PHP {data?.stat?.assets.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
        <div class="grid grid-cols-2 border-b px-2 py-3 mt-5">
          <span>Liquidity</span>
          <span class="font-bold">PHP {data?.stat?.liquidity.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) ?? 0}</span>
        </div>
            ...
            <table class="table w-full">
              ...
              {#each data?.sharesTransactions ?? [] as shareTransaction }
                <tr>
                    ...
                    <td>{shareTransaction.member?.givenName} {shareTransaction.member?.surname}</td>
          ...
          <table class="table w-full">
            ...
            {#each data.savingsTransactions ?? [] as savingsTransaction }
              <tr>
                  ...
                  <td>{savingsTransaction.member?.givenName} {savingsTransaction.member?.surname}</td>
            ...
            <table class="table w-full">
              ...
              {#each data.repayments ?? [] as repayment }
                <tr>
                    ...
                    <td>{repayment.loan?.member?.givenName ?? ""} {repayment.loan?.member?.surname ?? ""}</td>
        ...
        <div class="container bg-base-100 w-full  p-3 rounded">
          <OveviewTab stat={data.stat ??  fallbackStat} liquidityLogs={data.liquidityLogs} shareLogs={data.shareLogs} savingLogs={data.savingLogs}/>
        </div>
        {/if}
