...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    COALESCE(fl.interest, 0) + COALESCE(share.shares, 0)  + COALESCE(saving.savings, 0) as assets,
    ((COALESCE(fl.interest, 0) + COALESCE(share.shares, 0) + COALESCE(saving.savings, 0)) - COALESCE(dl.loan, 0) ) as liquidity
    FROM cooperative
    LEFT JOIN (
