...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    LEFT JOIN (SELECT member_id, SUM(remaining_balance) as total FROM loan where deleted_at is null and status = 'Disbursed' group by member_id) 
    as loan on member.id = loan.member_id
    LEFT JOIN (SELECT member_id, SUM(total_due) as total FROM loan where deleted_at is null and status = 'Finished' group by member_id) 
    as f_loan on member.id = f_loan.member_id
    LEFT JOIN (SELECT member_id, SUM(total_due) as total FROM loan where deleted_at is null and status = 'Requested' group by member_id) 
    as r_loan on member.id = r_loan.member_id
    LEFT JOIN (SELECT member_id, SUM(total_due) as total FROM loan where deleted_at is null and status = 'Approved' group by member_id) 
